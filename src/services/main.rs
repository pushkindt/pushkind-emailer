...
/// Service responsible for handling the main email workflows.
pub struct MainService<'a, R>
where
    R: EmailReader + EmailWriter + RecipientReader + EmailRecipientReader + GroupReader,
{
    repo: &'a R,
}

impl<'a, R> MainService<'a, R>
where
    R: EmailReader + EmailWriter + RecipientReader + EmailRecipientReader + GroupReader,
{
    ...
    pub fn load_index_page(
        ...
        let retry_email = match retry_id {
            Some(id) => self.repo.get_email_by_id(id, hub_id)?,
            None => None,
        };
        ...
        let (total, emails) = self.repo.list_emails(emails_query)?;
    ...
    pub async fn queue_new_email(
        &self,
        user: AuthenticatedUser,
        form: SendEmailForm,
        zmq_sender: &ZmqSender,
    ) -> ServiceResult<()> {
        let new_email = form.to_new_email(user.hub_id, self.repo)?;
    ...
    pub fn delete_email(&self, hub_id: i32, form: DeleteEmailForm) -> ServiceResult<()> {
        let email = self
            .repo
            .get_email_by_id(form.id, hub_id)?
            .ok_or(ServiceError::NotFound)?;

        self.repo.delete_email(email.email.id)?;
        Ok(())
    }

    /// Re-queues an email for delivery.
    pub async fn queue_email_retry(
        &self,
        hub_id: i32,
        ...
        let zmq_message = ZMQSendEmailMessage::RetryEmail((email.email.id, hub_id));
        zmq_sender.send_json(&zmq_message).await?;
        Ok(())
    }

    /// Marks a recipient as having opened an email.
    pub fn mark_email_opened(&self, recipient_id: i32) -> ServiceResult<()> {
        self.repo.update_recipient(
            recipient_id,
    ...
    pub fn export_email_recipients(
        &self,
        hub_id: i32,
        email_id: i32,
